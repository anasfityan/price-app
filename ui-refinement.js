(function(){
  ...
  function card(theme,label,kind,icon,subtitle){
    return '<div class="tswatch theme-choice '+kind+'-preview" data-th="th-'+theme+'" onclick="setThemeRow(this,\''+theme+'\')" role="button" tabindex="0" aria-label="'+label+'">'+
      '<div class="theme-choice-head">'+
        '<div class="theme-choice-label"><span class="theme-choice-icon">'+icon+'</span><span><strong style="display:block">'+label+'</strong><small style="display:block;font-size:8px;font-weight:600;color:var(--mu);margin-top:2px">'+subtitle+'</small></span></div>'+
  ...
  function rebuildThemeSelector(){
    ...
    darkRow.innerHTML =
      card('gold','داكن ذهبي','dark',iconMoon(),'فحمي · ذهب · برونز') +
      card('pearl','فاتح عاجي','light',iconSun(),'عاجي · برونز · ألوان واضحة');
    if(lightRow) lightRow.style.display='none';

    const darkLabel=darkRow.previousElementSibling;
    if(darkLabel){
      darkLabel.textContent='اختر هوية التطبيق';
