(function(){
  ...
  function normalizeTheme(theme){ return ALLOWED.has(theme) ? theme : FALLBACK; }
  function iconMoon(){return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 15.2A8.5 8.5 0 0 1 8.8 4 8.6 8.6 0 1 0 20 15.2Z"/></svg>';}
  ...
  function syncActive(){let stored=FALLBACK;try{stored=normalizeTheme(localStorage.getItem('theme')||FALLBACK);}catch(e){}document.querySelectorAll('.theme-choice').forEach(function(el){el.classList.toggle('active',el.dataset.th==='th-'+stored);});}
  function enforceAllowedTheme(){let stored=FALLBACK;try{stored=localStorage.getItem('theme')||FALLBACK;}catch(e){}if(!ALLOWED.has(stored)){try{localStorage.setItem('theme',FALLBACK);}catch(e){}if(typeof window.setTheme==='function') window.setTheme(FALLBACK);}}
  function wrapThemeSetter(){if(typeof window.setTheme!=='function'||window.setTheme.__twoThemeWrapped) return;const original=window.setTheme;const wrapped=function(theme){const normalized=normalizeTheme(theme);const result=original(normalized);setTimeout(syncActive,0);return result;};wrapped.__twoThemeWrapped=true;window.setTheme=wrapped;}
  function init(){wrapThemeSetter();enforceAllowedTheme();rebuildThemeSelector();}
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',init,{once:true}); else init();
  window.addEventListener('load',function(){wrapThemeSetter();enforceAllowedTheme();rebuildThemeSelector();},{once:true});
})();
